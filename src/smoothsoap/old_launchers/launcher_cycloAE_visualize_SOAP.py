...
if __name__=='__main__':
    trj1 = read_trj(data1, ':')
    trj2 = read_trj(data2, ':')
    trj = trj1[:250] + trj2[:250]
    #trj = trj1
    #trj = tamper_water_trj(trj)
    #trj = trj[:2]
    print('done loading the structures')
    dir = f'results/cycloAE/visual/CUTOFF/SOAP_deeptime_single/spatialavg'
    ...
    ids_atoms_train = [atom.index for atom in trj[0] if atom.number == centers[0]][:20]
    test_sigmas = [1, 2.5, 5]
    interval=1
    X_values = []
