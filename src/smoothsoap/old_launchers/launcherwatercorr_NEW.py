...
import os
data = '/Users/markusfasching/EPFL/Work/project-SOAP/scripts/SOAP-time-code/data/positions.lammpstrj'
...
if __name__=='__main__':
    ...
    trj1, ids_atoms = read_trj(data, format='lammps-dump-text', index='579000:590000')
    #trj2, ids_atoms2 = read_trj(data2)
    #trj = trj1 + trj2
    trj = trj1
    #trj = tamper_water_trj(trj)
    #import ase.io
    #ase.io.write('test.xyz', trj)
    #trj = trj[:10]
    ...
    dir = f'results/water_corr/NEW/SOAP_PCA/temporalCOV_PCA'
    dir = f'results/water_corr/NEW/SOAP_PCA/temporalGEV'
    dir = f'results/water_corr/NEW/SOAP_PCA/fullCOV_PCA'
    ...
    for interval in test_intervals:
        ...
        for i, cov in enumerate(cov):
            ...
            X=projected
            heights=np.array([atoms.positions[ids_atoms,2] for atoms in trj]).T
