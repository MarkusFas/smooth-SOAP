from src.smoothsoap.setup.read_data import read_trj, tamper_water_trj
from src.smoothsoap.old_scripts.descriptors import eval_SOAP, SOAP_COV, SOAP_COV_repair, SOAP_PCA, SOAP_COV_test
...
def run_simulation(methods):
    ...
    #trj = trj[:10]
    oxygen_atoms = [idx for idx, number in enumerate(trj[0].get_atomic_numbers()) if number==6] # only oxygen atoms
    ...
    for name in tqdm(methods, 'methods'):
        X_values = []
        for interval_idx, interval in enumerate(test_intervals):
            label = f'SOAP_cycloAE_interval_{interval}_r{SOAP_cutoff}_maxang{SOAP_max_angular}_maxrad{SOAP_max_radial}'
            ...
            """
            means = copy.deepcopy(MU[interval_idx])
            cov_i = copy.deepcopy(COV_I[interval_idx])
            cov_t = copy.deepcopy(COV_T[interval_idx])
            
            dir = f'results/cycloAE/CORRECT/SOAP_PCA/{name}'
            ...
            for i, cov in enumerate(cov_t):
                ...
                if name=='temporalGEV':
                    ...
                    pca_obj.compute_eigen(means[i], cov_i[i] + cov_t[i]) # old approach
...
if __name__=='__main__':
    methods = ['temporalGEV', 'temporalCOV_PCA', 'atomsCOV_PCA', 'fullCOV_PCA_old']
