...
def ridge_model_all(pcaA, pcaE, XA, XE, XA_trf_test, XE_trf_test, XA_clf_test, XE_clf_test, ids_atoms, nstepA, nstepE, scaler=None):
    models = []
    # pca C,N,S
    # all else also C,N,S
    ...
    print('y_pred', y_pred)
    print('y_true', y_clf_test)
    print('y_proba', CV_proba)
    print('brier loss', ll)
    #print('log loss: ', ll)
    return X_final_test, y_pred, y_clf_test, ll
...
def get_pca(soaps, ids_atoms, pca):
    ...
    return struct_soap_pca.reshape(62, soaps[0].shape[0], 3)
...
trajA_mol = aseio.read('prodA-mol.pdb', index='::2')[:]
print('done with A')
trajE_mol = aseio.read('prodE-mol.pdb', index='::2')[:]
print('done with E')
...
pca_raw = PCA(n_components=3).fit(Xraw)
...
Xpca, ypca, ypca_true = pca_model_all(pcaA, pcaE, gaussian_averageA, gaussian_averageE, timeseriesA_test_trf, timeseriesE_test_trf, timeseriesA_test_clf, timeseriesE_test_clf, ids_atoms_i, nstepA, nstepE, pca_raw)

print('avg2avg')
Xridge_avg2avg, yridge_avg2avg, yridge_avg2avg_true, ll_avg2avg = ridge_model_all(pcaA, pcaE, gaussian_averageA, gaussian_averageE, timeseriesA_test_trf, timeseriesE_test_trf, timeseriesA_test_clf, timeseriesE_test_clf, ids_atoms_i, nstepA, nstepE)
print('avg2raw')
Xridge_avg2raw, yridge_avg2raw, yridge_avg2raw_true, ll_avg2raw = ridge_model_all(pcaA_raw, pcaE_raw, gaussian_averageA, gaussian_averageE, timeseriesA_test_trf, timeseriesE_test_trf, timeseriesA_test_clf, timeseriesE_test_clf, ids_atoms_i, nstepA, nstepE)
print('raw2raw')
...
pca_raw = np.concatenate((pcaA_raw.transpose(1,0,2).reshape(-1,186),
                          pcaE_raw.transpose(1,0,2).reshape(-1,186)), axis=0)
pca_avg = np.concatenate((pcaA.transpose(1,0,2).reshape(-1,186),
                          pcaE.transpose(1,0,2).reshape(-1,186)), axis=0)

fig, ax = plt.subplots(62,7, figsize=(18, 200))
...
for i in range(len(ids_atoms)):
    ...
    ax[i,2].set_title(f'{i+1} pcatrafo')
...
plt.savefig('ridgeplots-alpha01.png')
