...
# Combined hydrogen species (HW1 + HW2)
hydrogen_indices = [i+1 for i, s in enumerate(symbols) if s in ("HW1", "HW2")]

# Oxygen species
oxygen_indices = [i+1 for i, s in enumerate(symbols) if s == "OW"]
print(types)
print(symbols)
selected_atoms = [
    i + 1 for i, (t, pos) in enumerate(zip(symbols, positions))
    if t == 'OW' and zmin <= pos[2] <= zmax
]
...
with open(plumed_fname, "w") as f:
    ...
    f.write(f"      SPECIES1={','.join(map(str, oxygen_indices))}\n")
    f.write(f"      SPECIES2={','.join(map(str, hydrogen_indices))}\n")
    f.write("      SPECIES_TO_TYPES=8,1\n")
